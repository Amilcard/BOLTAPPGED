...
## ⛔ RÈGLE #0 ter — GOUVERNANCE ANTI-RÉGRESSION (obligatoire, toutes sessions IA)

**Pourquoi** : le repo a connu une densité de bugs Thanh (test humain 2026-04-21) liée à 5 causes racines — pas d'E2E, IA multiples en série, pas de staging, data DB incomplète, handovers qui hallucinent. Les règles ci-dessous **tuent ces causes à la source**.

### Start-of-session — checklist OBLIGATOIRE (3 commandes, 30s)

Avant la 1re modification du repo dans une nouvelle session, exécuter **dans cet ordre** :

```bash
git log --oneline -10         # comprendre les 10 derniers commits
git status --short            # voir l'état non-commité
cat docs/TECH_DEBT.md         # voir la dette active
```

Si `git status` montre des fichiers non-commités que tu ne reconnais pas → **STOP**, demander à l'utilisateur avant toute action.

### End-of-session — DISCIPLINE OBLIGATOIRE

- Jamais de modifs non commitées à la fin d'une session. Soit commit, soit revert.
- Check final : `git status --short` doit être **vide** (ou seulement untracked explicites).

### Décisions techniques non-triviales → ADR 1 page

Toute décision qui satisfait l'un de ces critères → fichier `docs/adr/YYYY-MM-DD-<slug>.md` :

- Choix entre 2+ options viables aux conséquences différentes
- Impact auth / paiement / RGPD / data mineur
- Rollback d'une approche précédente
- Contrainte métier qui force un choix contre-intuitif

Format + index : `docs/adr/README.md`.

### Dette technique — budget 5 items max

- Fichier source de vérité : `docs/TECH_DEBT.md`
- Max 5 items actifs. Au-delà = freeze nouvelle feature.
- Toute rustine (`// FIXME:`, try/catch silencieux, `// TODO: hack`) → ligne ajoutée **le jour même** dans `docs/TECH_DEBT.md`.
- Pas de refacto "pour faire propre" — uniquement si bloquant.

### 1 IA = 1 zone à la fois (anti-collision multi-sessions)

Zones du repo :

- **auth** : `app/api/auth/*`, `lib/auth-*`, `middleware.ts`
- **payment** : `app/api/stripe/*`, `app/api/checkout/*`, tout ce qui touche `gd_inscriptions.stripe_*`
- **dossier** : `components/dossier-enfant/*`, `app/dossier-enfant/*`, signatures
- **admin** : `app/admin/*`, `app/api/admin/*`
- **educatif** : `app/educateur/*`, `app/sejour/*/souhait/*`, `app/api/souhaits/*`
- **infra** : `scripts/*`, `supabase/*`, `.github/*`, `CLAUDE.md`, `docs/*`

Règle : **une session IA déclare sa zone en ouverture** (1 ligne dans 1re réponse). Si besoin d'une 2e zone dans la même session → commit propre avant de changer de zone.

### Preview branch pour tests Thanh / E2E

- **Ne jamais tester sur la prod.** Avant toute session test utilisateur, lancer `./scripts/preview/create-branch.sh <nom>`.
- Après la session : `./scripts/preview/destroy-branch.sh <nom>` (sinon facturation Supabase continue).
- Workflow complet : `docs/PREVIEW_BRANCH.md`.

### Outils MCP obligatoires à des moments précis

| Moment | Outil MCP | Pourquoi |
|---|---|---|
| Avant commit feature touchant 3+ fichiers | `mcp__code-review-graph__get_impact_radius_tool` | Détecte cascade (fichiers impactés non-évidents) |
| Après chaque migration SQL | `mcp__c08868aa-...__get_advisors` | Alerte RLS cassées, indexes manquants, permissions |
| Après déploiement Vercel | `mcp__50dda79a-...__get_runtime_logs` | Remonte erreurs 5xx / React avant que user les voie |

### Hooks de garde automatiques

Le fichier `.claude/hooks/guard-destructive.sh` est branché en `PreToolUse/Bash`. Il bloque automatiquement :

- `rm -rf /` / `~` / `*` / `.`
- `git push --force` sur main
- `git reset --hard origin/main`
- `DROP TABLE` / `TRUNCATE TABLE` via bash
- `supabase db reset`
- `supabase branches delete` sur ref prod
- `cat .env` en clair

**Ne jamais contourner ces garde-fous** (ajouter `|| true`, changer le hook, etc.). Si un vrai besoin se présente : discuter avec l'utilisateur + ajouter une exception nominative + ADR.

### Anti-patterns formellement interdits (ajout à #0)

- ❌ Se fier à un handover doc (`docs/HANDOVER_*.md`) comme source de vérité sans vérifier le code réel
- ❌ Commit feature sans avoir couru le pre-commit hook (TSC + ESLint + Jest)
- ❌ Laisser une branch Supabase preview active > 24h sans raison explicite
- ❌ Tester un changement UX/fonctionnel directement en prod
- ❌ Amender un commit déjà pushé sur main
- ❌ Créer un fichier "plan" / "audit" / "handover" long en markdown quand 5 lignes dans `docs/adr/` suffisent
...
`gd_inscriptions`, `gd_dossier_enfant`, `gd_propositions_tarifaires`, `gd_factures`, `gd_incidents`, `gd_medical_events`, `gd_calls`, `gd_notes`, `gd_structure_access_codes`, `gd_educateur_emails`, `gd_stay_sessions` (si affecte dossier), `gd_souhaits`, `smart_form_submissions`.
