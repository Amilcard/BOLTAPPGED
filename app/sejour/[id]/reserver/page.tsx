...
export default async function ReserverPage({ params, searchParams }: PageProps) {
  ...
  // Note: gd_stay_sessions n'a pas de colonne seats_left
  // → seatsLeft = -1 signifie "donnée indisponible" (le front n'affiche pas "complet")
  const sessions = sessionsRaw.map((s: any) => ({
    ...s,
    startDate: s.start_date,
    endDate: s.end_date,
    seatsLeft: s.seats_left ?? -1, // -1 = donnée non disponible, jamais bloquer
  }));
