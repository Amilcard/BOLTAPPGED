...
import { CheckInstructions } from './check-instructions';
...
export function BookingFlow({ stay, sessions, initialSessionId = '', initialCity = '' }: BookingFlowProps) {
  ...
  const departureCities: DepartureCity[] = (stay as any).departureCities || [];
  const enrichmentSessions: SessionPriceData[] = (stay as any).enrichmentSessions || [];
  ...
  const [paymentMethod, setPaymentMethod] = useState<'stripe' | 'transfer' | 'check' | null>(null);
  ...
  if (selectedSession) {
    if (enrichmentSessions && enrichmentSessions.length > 0) {
      ...
      // Fallback si pas de data enrichie
      sessionBasePrice = (stay as any).priceFrom || 0;
    }
  }
  ...
  const validSessions = sessions?.filter(s => s?.seatsLeft == null || s.seatsLeft < 0 || s.seatsLeft > 0) ?? [];
  ...
  const isStep1Valid = step1.addresseStructure && step1.addresseStructure.trim().length >= 5 && step1.organisation && step1.socialWorkerName && step1.email && step1.phone;
  ...
  const handleSubmit = async () => {
    if (!selectedSessionId || !selectedSession) {
      setError('Session non sélectionnée');
      return;
    }

    if (!selectedCity) {
      setError('Ville de départ non sélectionnée');
      return;
    }
    ...
    // ✅ VALIDATION ÂGE CRITIQUE - Bloquer inscriptions hors tranches
    const childAge = calculateAge(step2.childBirthDate);
    const ageMin = stay?.ageMin || 6;
    const ageMax = stay?.ageMax || 17;
    ...
    if (childAge < ageMin || childAge > ageMax) {
      setError(
        `✨ Ce séjour est conçu pour les ${ageMin}-${ageMax} ans, et ${step2.childFirstName} a ${childAge} ans. ` +
        `Mais ne t'inquiète pas ! D'autres aventures extraordinaires l'attendent. ` +
        `🌟 Explore nos autres séjours pour trouver celui qui lui correspondra parfaitement !`
      );
      return;
    }
    ...
    try {
      ...
      const remarques = [
        step1.addresseStructure ? `Adresse: ${step1.addresseStructure}` : '',
        step2.childSex ? `Sexe: ${step2.childSex}` : '',
      ].filter(Boolean).join('\n');
      ...
      if (!res.ok) {
        ...
        throw new Error(data?.error?.message || data?.error?.details || 'Erreur lors de la réservation');
      }

      setBookingId(data?.id ?? '');
      setInscriptionId(data?.id ?? '');
      setPaymentReference(data?.payment_reference ?? '');
      // Aller à étape 5 : choix mode paiement (au lieu de succès direct)
      setStep(5);
    } catch (err: unknown) {
  ...
  return (
    <div className="p-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex-1">
                  <div className="flex items-center gap-2 text-sm text-primary-700 mb-1">
                    ...
                    <span>
                      {selectedSession?.startDate ? formatDateLong(selectedSession.startDate) : '...'} - {selectedSession?.endDate ? formatDateLong(selectedSession.endDate) : '...'}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 flex justify-between items-center">
                    <div>
                      ...
                      <div className={`text-xs ${isFull ? 'text-red-500' : 'text-primary-500'}`}>
                        {isFull ? 'Complet' : (session?.seatsLeft != null && session.seatsLeft >= 0) ? `${session.seatsLeft} places restantes` : 'Places disponibles'}
                      </div>
                    </div>
                    {displayPrice && !isFull && (
        ...
        <div className="space-y-4">
          ...
          <div className="bg-primary-50 p-4 rounded-xl space-y-2 border border-primary-100">
            ...
            <div className="space-y-1 text-sm text-primary-700">
              ...
              <p>
                <span className="font-medium text-primary-500">Session :</span>{' '}
                {selectedSession?.startDate ? formatDateLong(selectedSession.startDate) : '...'} - {selectedSession?.endDate ? formatDateLong(selectedSession.endDate) : '...'}
              </p>
      ...
      {/* Step 5: Choix mode de paiement */}
      {step === 5 && (
        <div className="space-y-4">
          <h3 className="font-medium text-primary text-lg">Étape 6/7 : Paiement</h3>
          <PaymentMethodSelector
            onSelectStripe={async () => {
              setLoading(true);
              try {
                ...
                // Stripe Checkout — pas encore configuré en production
                if (data.clientSecret) {
                  setError('Le paiement par carte bancaire sera disponible prochainement. Merci de choisir le virement ou le chèque.');
                }
              } catch (err) {
      ...
      {/* Step 6: Instructions paiement */}
      {step === 6 && paymentMethod === 'transfer' && (
        <div className="space-y-4">
          <h3 className="font-medium text-primary text-lg">Étape 7/7 : Instructions virement</h3>
      ...
      {step === 6 && paymentMethod === 'check' && (
        <div className="space-y-4">
          <h3 className="font-medium text-primary text-lg">Étape 7/7 : Instructions chèque</h3>
          <CheckInstructions
            amount={totalPrice ?? 0}
            paymentReference={paymentReference}
            childName={step2.childFirstName}
            stayName={stay?.marketingTitle || stay?.title || ''}
            onClose={() => setStep(7)}
          />
        </div>
      )}
      ...
      {step === 7 && (
        <div className="text-center py-6">
          ...
          <div className="bg-primary-50 p-4 rounded-xl text-left text-sm space-y-1">
            <p><strong>Référence :</strong> {paymentReference || bookingId}</p>
            ...
            {paymentMethod && (
              <p>
                ...
                {paymentMethod === 'transfer' && 'Virement bancaire (2-3j)'}
                {paymentMethod === 'check' && 'Chèque (5-7j)'}
              </p>
            )}
          </div>
          <button
